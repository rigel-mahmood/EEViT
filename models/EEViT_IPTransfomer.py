...
class EEViT_IPTransformer(nn.Module):
    def __init__(
        self,
        *,
        ...
        for i in range(num_layers):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, EEViT_IPAttention(dim = dim, heads = heads, sequence_len = sequence_len, segment_len = segment_len, num_segments=num_segments, 
        ...
        self.to_logits_0 = nn.Sequential(
            nn.LayerNorm(dim),
    ...
    def forward(self, x, mask = None):
        ...
        for attn, ff in self.layers:
            ...
            x = ff(x) + x   
        out0 = self.to_logits_0(x[:,0:16,:])
        out1 = self.to_logits_1(x[:,-16:,:])
        out = torch.mean(torch.cat([out0,out1],dim=1),dim=1)   
        return out 
